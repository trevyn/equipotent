...
pub async fn instant_search(query: String) -> anyhow::Result<Vec<SearchQueryResultItem>> {
 ...
 Ok(select!(Vec<SearchQueryResultItem> r#"
  ...
  FROM (
   SELECT
    highlight(resultitem_fts, 1, '<span class="search-highlight-url">', '</span>') AS search_highlighted_url,
